...
2. The manuscript roots configured in `runtime-profile.yaml`.

Metadata verification and claim support are separate operations. A correct DOI and author list do
not establish that a manuscript sentence is supported by the cited work; claim support must be
audited against the version and source rung actually read.
...
Create `<bib_dir>/bibliography-audit_<YYYYMMDD_HHMM>/` containing `entries/` (one `.bib` fragment per entry,
...
`<bib_dir>/bibliography-audit_<YYYYMMDD_HHMM>/bibliography-audit_report.md`:
...
`% bibliography-audit:FAIL <diagnostic>` comment line.
...
`the file named by SCHOLAR_ENV_FILE`): fall back to Crossref-only checking, which still covers checks 1
