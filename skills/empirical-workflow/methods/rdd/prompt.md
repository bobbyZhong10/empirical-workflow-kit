...
An opinionated RD workflow grounded in a read canon (canon.md, current as of
...
mechanics, thresholds, and what a narrow window does to power: details.md.
...
- Extrapolation beyond the cutoff LATE needs added assumptions, and the menu is in
  details.md. Say which one when claiming anything away from the cutoff. Absent one,
  the methods paragraph says the estimate is local to the cutoff, full stop.

## R implementation

The complete runnable pipeline is template.R (estimation in both frameworks, fuzzy
...
Package index with versions and links in details.md. Stata and Python mirrors of the
