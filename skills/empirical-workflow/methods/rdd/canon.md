...
- Verified through: 2026-07-28, using the source and version notes recorded below.
- Role: authority for continuity- and local-randomization-based regression discontinuity.
- Settles: local-polynomial defaults, bandwidth/inference rules, and manipulation/covariate diagnostics.
- Binds when: a known threshold rule assigns treatment through a running variable.
- Implement: use `template.R` with the locked cutoff, score construction, and analysis window.
- Scope limits: the estimand is local; fuzzy designs inherit IV assumptions at the cutoff.
- Named disagreements: continuity and local-randomization frameworks have distinct scopes retained below.
- Excluded: high-order global polynomials and post-result cutoff or bandwidth selection.
...
- Implement: the verbatim R workflow block (reproduced in template.R); replication
