...
hand-picked sources (see canon.md, current as of 2026-08-05) spanning two
...
- Every AMCE names its averaging distribution. The uniform AMCE (uAMCE) and the
  ...
  check in details.md, or the global F-test below) or argues a theoretically
...
- Randomization: uniform independent randomization is the parsimony default, with three
  ...
  2022; ladder details and the ESS formula in details.md).
...
- Multiple testing: never report an uncorrected forest of AMCE stars. Under a global
  ...
  (../causal-design/shared-rules.md). Plain BH and not the adaptive BKY variant,
  deliberately: BKY's
...
- Proportion claims gate through the AKM sharp bounds, which need only the AMCE, the
  ...
  details.md; quick screens: a binary attribute in a large design needs an
...
- The stack is choice-based conjoint estimated by hierarchical Bayes, the field's
  ...
  details.md; the template implements them.
- The sign-constraint hard rule: any run constraining a coefficient's sign (price
  ...
  the discrepancy table is in details.md). Report the priors used.
...
- Managerial translation (MVAI and its cost threshold, reservation-price pricing,
  product-line optimization) is mapped in details.md. Decisions ride on the
...
- AMCE interpretation (Abramson-Kocak-Magazinnik 2022 vs Bansak-Hainmueller-Hopkins-
  ...
  applied prose. Full adjudicated cell with the asymmetries in details.md.
...
fallbacks: details.md. Read it before writing any package call. The template
(template.R) runs the full randomized-experiment pipeline and the HB block,
