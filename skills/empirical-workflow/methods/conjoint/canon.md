...
- Role: authority for randomized-profile conjoint design, estimands, diagnostics, and interpretation.
- Settles: AMCE/AMCE-style estimands, profile randomization, respondent dependence, and design restrictions.
- Binds when: attributes or profiles are randomized and choices or ratings identify effects.
- Implement: use the design checks and estimators in `template.R` with respondent-level inference.
- Scope limits: effects depend on the attribute distribution and profile restrictions actually fielded.
- Named disagreements: descriptive preference heterogeneity is distinct from causal moderation.
- Excluded: infeasible profiles, hidden restrictions, and population claims beyond the sampled respondents.
...
../causal-design/references.bib, the family's shared bib. Refresh: literature-review on
