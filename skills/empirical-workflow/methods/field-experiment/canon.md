...
- Role: authority for randomized field designs, covariate adjustment, attrition, and interference.
- Settles: design-based estimands, valid adjustment, assignment-consistent inference, and spillover routing.
- Binds when: treatment is randomized in a field, cluster, network, or marketplace setting.
- Implement: encode the randomization and estimand in `template.R` before outcome analysis.
- Scope limits: noncompliance, attrition, interference, and adaptive assignment require their own estimands.
- Named disagreements: model-assisted precision choices cannot override the randomization design.
- Excluded: post-randomization controls, outcome-driven subgrouping, and naive analysis of adaptive samples.
...
BibTeX keys point into ../causal-design/references.bib. Refresh: literature-review on the
...
- Implement: glm + marginaleffects::avg_comparisons (our mapping); the four-line per-arm
  imputation pattern in template.R.
...
- Implement: Stata leebounds (Tauchmann) is the standard; R is hand-rolled or the Semenova
  GitHub package (state verified in details.md); the template hand-rolls with a
  bootstrap.
