...
- Role: authority for donor-based counterfactuals with one or a few treated aggregate units.
- Settles: feasibility, pre-treatment fit, donor construction, placebo inference, and walk-away rules.
- Binds when: aggregate treatment and a credible donor pool identify a counterfactual path.
- Implement: use `template.R` only after the donor and pre-period gates pass.
- Scope limits: poor fit, interpolation failure, spillovers, or short pre-periods can make the design infeasible.
- Named disagreements: method and inference choices are resolved by the data-shape conditions below.
- Excluded: choosing donors, predictors, or fit windows after inspecting focal post-treatment gaps.
