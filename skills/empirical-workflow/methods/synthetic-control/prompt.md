...
An opinionated synthetic-control workflow grounded in a read canon (canon.md,
...
details.md carries what each case is the precedent for.
...
- Synthetic DiD is SC with unit weights plus analogous time weights and a DiD-style
  ...
  exists yet. The four assumptions SDID actually makes are in details.md.
...
- Treated outcome outside the donor convex hull: augmented DiD (Li and Van den Bulte 2023),
  ...
  pre-periods: HCW OLS (Hsiao, Ching, and Wan 2012). Fuller map in details.md.

## R implementation

The complete runnable pipeline is template.R (canonical SC with the full
...
Package index with versions, links, and traps in details.md.
