...
- Verified through: 2026-08-04, using the source and version notes recorded below.
- Role: authority for instrument validity, strength, estimands, and weak-IV inference.
- Settles: LATE interpretation, first-stage reporting, exclusion arguments, and robust inference routes.
- Binds when: an excluded source of treatment variation is the identifying design.
- Implement: use the diagnostics and reference estimators in `template.R` after locking the instrument set.
- Scope limits: exclusion and monotonicity remain substantive assumptions rather than empirical tests.
- Named disagreements: competing strength diagnostics and estimators are adjudicated by conditions below.
- Excluded: selecting instruments from focal outcomes or treating a large conventional F alone as validity.
...
../causal-design/references.bib. Refresh: literature-review on the method since the date
...
- Implement: ssaggregate (Stata/R), bartik_weight for Rotemberg weights, ShiftShareSE for AKM;
  details and traps in details.md.
...
- Implement: the paper ships no code (AMPL plus Gurobi); the practical route is the ivmte R
  ...
  details.md, optional template block in template.R.
...
- Implement: the authors' own R package ManyIV (github.com/kolesarm/ManyIV), row in
  details.md, template block in template.R. Chyn-Frandsen-Leslie 2025
