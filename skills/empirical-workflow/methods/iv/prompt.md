...
An opinionated IV workflow grounded in a read canon (canon.md, current as of
...
case teaches, are in details.md.
...
market numbers in details.md are the two-line demonstration). An instrument is an
...
- Hold instruments to a robust first-stage F of about 50 (about 50/K^(3/4) with K instruments),
  ...
  in details.md. This bar prices 2SLS bias, so it moves with the estimator: it does
...
it lands close. The trace algebra is in details.md.
...
1. Robust first-stage F, computed at the level the design lives at: in-bandwidth for fuzzy RD
   ...
   Read it against the ladder in details.md, not against 10. Leniency designs are
...
3. Balke-Pearl inequality checks (binary Y, X, Z): four testable inequalities implied by the
   ...
   nothing. Worked flu-data numbers in details.md.
...
The complete runnable pipeline is template.R (estimation, the full weak-IV inference
...
Every claim traces to canon.md; keys live in ../causal-design/references.bib.
