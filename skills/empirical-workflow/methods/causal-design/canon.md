...
- Verified through: 2026-08-05, using the source and version notes recorded below.
- Role: assumptions-first router and shared inference authority for the method family.
- Settles: the assignment taxonomy, method-family boundaries, and common clustering rule.
- Binds when: selecting or rejecting a causal design before a focused pack is loaded.
- Implement: route to one selected pack; use this pack's template only for its stated branch.
- Scope limits: routing cannot establish a design whose substantive assumptions are unsupported.
- Named disagreements: retain the source-specific disagreements and revisions documented below.
- Excluded: no estimator shopping, retrospective relabeling, or silent change to a locked design.
...
approval. BibTeX keys point into `references.bib`, which is the
...
run literature-review on the moving corners (panel estimators, text-causal) since the canon date;
