...
An opinionated DiD workflow grounded in a read canon (canon.md, current as of
...
Refresh path: run literature-review on "difference-in-differences" since the canon date, then propose
additions to canon.md as flagged addenda.

## Exemplar designs

Seven shapes worth recognizing on sight. Find the one your data looks like before writing a
specification; what each one teaches is in details.md.
...
3. **Do you have many treated and untreated clusters?** Yes: cluster at the level at which
   ...
   assumption you believe (details.md has the map), or fall back to a cluster-level
...
- Report simultaneous sup-t uniform bands, computed separately for leads and lags, not only
  pointwise bands (recipe in details.md; the did package produces them by multiplier
...
details.md separates what each does to pre-trends from what it does to PT.
...
year dummies (both in details.md), clustered at the level treatment was applied.
...
the map in details.md lists each option with the homogeneity assumption it needs,
...
Full argument: ../causal-design/shared-rules.md.
...
The complete runnable pipeline is template.R: estimation, both HonestDiD
...
Every claim in the paragraph must trace to a canon entry; canon.md maps claims to
papers and BibTeX keys in the shared ../causal-design/references.bib. Verify any primary
paper cited beyond the canon with bibliography-audit before submission.
