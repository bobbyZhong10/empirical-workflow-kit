...
- Verified through: 2026-08-26, using the source and version notes recorded below.
- Role: authority for DiD, event-study, and DDD design and inference.
- Settles: estimator choice under timing heterogeneity and the required assumption/diagnostic battery.
- Binds when: treatment timing and credible untreated or not-yet-treated comparisons identify effects.
- Implement: follow the estimator/package crosswalk and `template.R` after locking the estimand.
- Scope limits: failed parallel trends, anticipation, spillovers, or unsupported comparisons can stop the design.
- Named disagreements: defaults and conditions for departing from them are preserved below.
- Excluded: staggered-adoption TWFE as the main result without a defended homogeneity argument.

Current as of 2026-08-26. These sources are hand-picked; nothing enters this file without
explicit human approval. BibTeX keys point into ../causal-design/references.bib.
Refresh: literature-review on the method since the date above, results proposed as flagged addenda.
...
re-checked with bibliography-audit at manuscript time. Keys: `goodmanbacon2021timing` (decomposition);
