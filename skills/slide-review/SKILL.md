...
Same loop as `latex-production --figures`: render, rasterize, let an agent with eyes judge the image,
...
this skill's directory. `$ASSETS` is `$EWF_PRESENTATION_ASSETS`, and its `README.md` is the
...
Treat an authenticated browser as the serialized singleton declared in `runtime-profile.yaml`. Two agents driving it can hijack each
...
If the positional is a bare filename, search the current project before asking. Read
`runtime-profile.yaml` for browser, state, PDF, Node, and presentation-tool commands.

## Setup

```bash
ASSETS="$EWF_PRESENTATION_ASSETS"
RUN="$EWF_STATE_DIR/slide-review/$(date +%Y%m%d-%H%M%S)"
mkdir -p "$RUN/shots"
...
Use only PDF and image commands declared in the runtime profile. Every PDF goes through the
configured helper, including any PDF used for visual review.
...
Route every PDF through the helper declared in `runtime-profile.yaml`, then inspect the PNG:

```bash
$EWF_PDF_HELPER png "$RUN/deck.pdf" --pages 3 --dpi 110 --out "$RUN/shots/s"
```
...
```bash
$EWF_PDF_HELPER pages deck.pdf      # page count first
$EWF_PDF_HELPER png deck.pdf --pages 1-24 --dpi 110 --out "$RUN/shots/slide"
```
...
pixel size with a configured image-inspection tool; under 1400 wide, re-render at `--dpi 150` so the
reviewers can judge small type. Correct once, do not loop. Use the configured Ghostscript fallback
when the primary PDF helper is unavailable.
...
Judging whether the research is right. That is `/manuscript-review`.
