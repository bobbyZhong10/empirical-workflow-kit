---
name: replication-release
description: Assemble a journal-ready replication archive for a quantitative-marketing paper, scanning for secrets, PII, and absolute local paths before the zip is built. TRIGGER on "/replication-release", "build a replication package", "data availability statement", "zip up the code and data for submission", a data-editor email asking what to send, or any "gather everything a replicator would need" request.
---
...
Before packaging, reverify each target journal's current data, code, disclosure, and repository
policy from an official source. Record the policy URL and `verified_at` date; the examples below
are operational defaults, not timeless authority. A policy change is a release-gate input.

Confidential, licensed, restricted, human-subject, and contractual data require an explicit
applicability and release decision. Never infer permission from local possession. If redistribution
is not authorized, ship a documented access procedure or synthetic/minimal substitute only when it
is complete enough to satisfy the governed release gate.

## Discovery and runtime profile

Resolve manuscript roots, document helpers, and local tool commands from `runtime-profile.yaml`.
Project-specific code root and archive destination remain explicit inputs and are never guessed.
...
| Paper root | The path argument if given, else the configured manuscript roots; ask the user to pick when several match. |
...
PDFs turn up here as preregistrations, IRB approvals, and pasted data-editor letters. Route every
PDF through the helper configured in `runtime-profile.yaml`:

```bash
$EWF_PDF_HELPER text prereg.pdf
$EWF_PDF_HELPER text prereg.pdf --pages 1-5
$EWF_PDF_HELPER png prereg.pdf --pages 1 --dpi 150 --out /tmp/p   # then Read /tmp/p-1.png
```

If no PDF helper is configured, stop and request a text or image export.

## Two roots, both given as input

A package needs two directories, and in this setup they are normally in different places.

Paper root is a configured manuscript project holding `main*.tex`, `.bib`, `Tables/`, and
`Figures/`. A paper root with only manuscript and exhibit assets may be valid. Read it for the
exhibit list, title, abstract, and author block; never copy the manuscript into the archive.
...
Hard rule: the archive is never written under any manuscript or Overleaf-sync root declared in
`runtime-profile.yaml`. Refuse the path before creating it. Test the user-supplied, logical, and
physical paths because a symlink may otherwise bypass the configured-root check:

```bash
# Compare DEST against every configured manuscript and sync root before mkdir.
...
# Compare DEST_LOG and DEST_ABS against every configured manuscript and sync root.
...
All sanitization occurs in staging copies. The source tree is read-only for this workflow and is
never overwritten, even when a finding is mechanically repairable.
...
- A PDF needs reading: use `$EWF_PDF_HELPER`. In this setup the Read tool fails on PDFs and `pdftotext` and `pdftoppm` do not exist.
