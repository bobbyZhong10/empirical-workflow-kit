...
Before reviewing a third party's submission, complete the protocol's policy and confidentiality
check. If the journal, institution, agreement, or consent terms do not authorize transfer to the
active model or service, stop. Record the authority and permitted scope in the review request.
...
1. An explicit path argument wins. Route PDFs through the helper configured in
   `runtime-profile.yaml`:
   `$EWF_PDF_HELPER text paper.pdf` for the text, or
   `$EWF_PDF_HELPER png paper.pdf --pages N --dpi 150 --out /tmp/p` and then Read
   the PNG when the layout or a figure matters.
2. No path: Glob `**/*.tex` in the working directory.
3. Still nothing: search the manuscript roots configured in `runtime-profile.yaml`, show the
   matching project directories, and ask which one. Do not guess between projects.

In referee mode, step 1 is the only step: skip steps 2 and 3 and ask for the path if none was
given.

When parallel workers are unavailable or not authorized, execute the same six lenses sequentially
into separate immutable findings files. Do not let later lenses read earlier verdicts. Consolidate
only after all requested lenses finish; consequence, not vote count, sets priority.
...
> `$EWF_PDF_HELPER text <file.pdf>` (add `--pages 1-20` to work through a long
...
`$EWF_PDF_HELPER png fig.pdf --dpi 200 --out /tmp/fig` and Read the PNG. Look at
