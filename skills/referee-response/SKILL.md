...
Read `runtime-profile.yaml` and search its configured manuscript roots. Match the user's wording
against directory names; ask once if two fit. Then read what is
actually there:
...
Use the PDF text command configured in `runtime-profile.yaml`. When `EWF_PDF_HELPER` is bound, run
`$EWF_PDF_HELPER text letter.pdf`. Do not assume a platform-specific PDF utility.

If that returns almost nothing the letter is a scan (`pdfread.py pages letter.pdf` says so). Then
rasterize it, `$EWF_PDF_HELPER png letter.pdf --dpi 200 --out /tmp/letter`, and
...
A `.docx` letter uses the configured `docx_text_command`. If none is available, request a PDF or
plain-text export instead of guessing at the content.
...
Verify every pin before drafting the corresponding response. Record the manuscript file, section,
line or stable anchor, exact version identifier, and verification time in the response matrix.
...
5. Any reviewer comment citing a paper the manuscript does not engage with, so the bibliography
   can be fixed before submission. Use the `research-sources` skill to resolve those references
