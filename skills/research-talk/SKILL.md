...
and the generator pattern; run the `bibliography-audit` skill on the `.bib` before the
talk.
...
cp -R $EWF_PRESENTATION_ASSETS/{mathjax,fonts} .
```
...
node $EWF_PRESENTATION_ASSETS/deck-check.mjs fit deck.html   # must print DECK-FITS: YES
node $EWF_PRESENTATION_ASSETS/stage-check.mjs deck.html      # must print STEP-0-CLEAN: YES

# self-contained variant only; verdict line is the padded label OFFLINE-SAFE
$EWF_PRESENTATION_ASSETS/check-offline.py deck.html
```
...
$EWF_PDF_HELPER png one.pdf --pages 1 --dpi 110 --out /tmp/s   # /tmp/s-1.png
...
Use the configured PDF helper. If the active browser profile is serialized, prefer decktape and
serialize any browser-assisted fallback.
...
```bash
node $EWF_PRESENTATION_ASSETS/deck-check.mjs handout deck.html deck-handout.pdf
```
