...
Structure and the MUST/SHOULD/MAY taxonomy descend from
...
Retrospective preregistration is prohibited.
...
the same check `bibliography-audit` runs, at prereg scale (a handful of citations against a `.bib` of
hundreds), and it uses bibliography-audit's verdict vocabulary: PASS/WARN/FAIL. For a full `.bib` audit, use
the `bibliography-audit` skill.

Claim support: does the cited paper actually support the sentence citing it? This check lives
here; bibliography-audit declares it out of scope, since bibliography-audit verifies metadata only.
...
Resolve `<project>` from the active `research.yaml`. Optional manuscript roots
come from `runtime-profile.yaml`. If neither identifies a destination, ask for
the project path rather than writing into a cache directory.
...
Related: the `research-sources` skill resolves and reads any paper cited here;
`latex-production` builds the `.tex` output if `--format tex` was used.
