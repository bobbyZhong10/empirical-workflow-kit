...
`skills/research-sources/SKILL.md` before the reading phase if you have not
...
```bash
skills/research-sources/scripts/paper.py search "<topic>" -n 25 --json
```
...
Reading is where the value is, and the research-sources skill documents the pattern: one subagent per
...
Tool: skills/research-sources/scripts/paper.py
Follow skills/research-sources/SKILL.md for the access ladder.
...
source coverage line: which sources answered, which failed, which venues were unreadable, and what the
...
prices are all in `skills/research-sources/SKILL.md`. Name in the output every
