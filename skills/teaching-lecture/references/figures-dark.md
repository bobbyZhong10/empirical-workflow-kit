...
source was found for this media" modal. Use the media command declared in the runtime profile. If
none is declared, `uv run --with imageio-ffmpeg python -c "import imageio_ffmpeg as
f; print(f.get_ffmpeg_exe())"` can provision a temporary static binary.
