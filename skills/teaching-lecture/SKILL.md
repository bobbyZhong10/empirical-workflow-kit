...
not write (use `slide-review`), and LaTeX Beamer (use `latex-production`). The
`course-site` skill owns everything around the decks: the landing page, the week
table, the site theme, publishing.

Shared theme and tooling live in `$EWF_PRESENTATION_ASSETS/`.
...
```bash
cd <deck dir>
quarto add $EWF_PRESENTATION_ASSETS --no-prompt   # installs _extensions/starter/
cp -R $EWF_PRESENTATION_ASSETS/{mathjax,fonts} .  # the format self-hosts MathJax and both faces at relative urls
```
...
node $EWF_PRESENTATION_ASSETS/deck-check.mjs fit lecture.html   # must print DECK-FITS: YES
node $EWF_PRESENTATION_ASSETS/stage-check.mjs lecture.html      # must print STEP-0-CLEAN: YES

# only if this deck was built as the offline variant
$EWF_PRESENTATION_ASSETS/check-offline.py lecture.html           # must print OFFLINE-SAFE: YES
```
...
```bash
node $EWF_PRESENTATION_ASSETS/deck-check.mjs handout \
...
## Traps verified in the upstream toolchain
...
| Math unrendered even with wifi | the format's self-hosted `mathjax/MathJax.js` url resolves to nothing next to the rendered HTML | copy `$EWF_PRESENTATION_ASSETS/mathjax/` beside the deck, or override the url as the course project does |
| Type looks wider than the fit gate measured | no `fonts/` copy next to the deck, so both faces fell back to the system stacks | copy `$EWF_PRESENTATION_ASSETS/fonts/` beside the deck, or override the four @font-face urls |
...
| `$EWF_PRESENTATION_ASSETS/README.md` | The machinery: the filter's six jobs, staging internals, both gates, citations and refs packing, jump buttons, the progress bar, the silent breakers, handouts, fonts. |
