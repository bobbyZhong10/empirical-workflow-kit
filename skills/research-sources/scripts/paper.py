...
Adapted from ericluo04/claude-academic-workflow at commit
...
# Optionally load KEY=VALUE lines from SCHOLAR_ENV_FILE; real env vars win.
_secrets_file = os.environ.get("SCHOLAR_ENV_FILE")
SECRETS = Path(_secrets_file).expanduser() if _secrets_file else None
if SECRETS is not None and SECRETS.exists():
...
CACHE = Path(os.environ.get("PAPER_CACHE", _cache_root / "empirical-workflow" / "papers"))
...
def _absorb(dst: dict, src: dict) -> None:
    ...
    for name, cnt in (src.get("citations") or {}).items():
        if cnt is not None:
            dst.setdefault("citations", {})[name] = cnt
    dst["sources"] = list(dict.fromkeys(
        list(dst.get("sources") or []) + list(src.get("sources") or [])
    ))
    ...
    try:
        ...
        if len(sample.strip()) < 200:
            return (f"PDF {url} (SCANNED)",
                    ...
                    " see the OCR rung in the research-sources skill.")
    except Exception:
        pass
