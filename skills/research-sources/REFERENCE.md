...
3. Born-digital PDF: real text layer; read it with
   `$EWF_PDF_HELPER text <pdf>` (`png` for figure pages).
4. OCR, only for scans, through the `the configured OCR project` pipeline (if you have one set up).
...
All keys go in `the file named by SCHOLAR_ENV_FILE` (chmod 600; real env vars override it).
...
- OCR at scale on an HPC cluster: `the configured OCR project` is wired in as the scanned-PDF rung (this
