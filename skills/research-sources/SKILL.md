...
Keys (optional, all free) live in `the file named by SCHOLAR_ENV_FILE`; the script auto-loads them.
...
Every retained note must record `version_read`, `source_rung`, and source coverage. If the best
available record is abstract only, label it `abstract only` and limit every substantive claim to
what the abstract supports. Never imply full-text coverage from metadata, snippets, or an abstract.
...
is reported and skipped, never fatal. For a whole literature, use the `literature-review` skill, which
...
4. Born-digital PDF (NBER, JMLR, PMLR, repositories): saved to `the configured research cache/`;
   read with `$EWF_PDF_HELPER text <pdf>` (`png` for figure pages).
...
### The OCR rung: `the configured OCR project` (scanned PDFs only)
...
`the configured OCR project` repo
(an OCR pipeline on an HPC cluster); do not hand-roll OCR.

- Read first: `the configured OCR project/README.md`, `docs/ocr-engines.md`, and the relevant
