...
Read `runtime-profile.yaml`. In order: use an absolute or cwd-relative path if given;
otherwise `Glob` the manuscript roots configured in `runtime-profile.yaml` and, if a
...
4. Resolve `latexmk` and Ghostscript from the commands configured in `runtime-profile.yaml`.
   If the LaTeX command is unavailable, fail with `SETUP_MISSING:latexmk`.
   With `--figures`, also check the configured Ghostscript command; if absent, report
...
State lives at `$EWF_STATE_DIR/latex-production/<hash>/last.json`, where `<hash>`
...
first, since the configured state directory may not exist yet. With no baseline,
...
```bash
RUN="$EWF_STATE_DIR/latex-production/figures/$(date +%Y%m%d-%H%M%S)"
mkdir -p "$RUN/fig-01/iter-01"   # and iter-NN at the top of every later round
cd "$RUN/fig-01/iter-NN" && "$EWF_LATEXMK" -pdf -interaction=nonstopmode -halt-on-error diagram.tex
```
...
`$EWF_PDF_HELPER text <outdir>/<jobname>.pdf`. This setup assumes
...
```
latex-production "Algorithmic Pricing Manuscript" --box-threshold=2pt
